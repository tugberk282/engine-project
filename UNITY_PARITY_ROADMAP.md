...
| Hierarchy display and selection | **Partial** | Hierarchical rows, search, multiselect, context menus, rename and drag/drop code exist in `HierarchyWindow.ts`. TUG-102 independently reproduced one source-built Windows Electron run in which Ctrl+Shift+N created/selected an object, F2/Enter renamed it and returned tree focus, the keyboard context-menu key moved focus into the menu, and Escape dismissed it and restored tree focus. This synthetic in-page workflow does not cover pointer/keyboard selection matrices, expand/collapse, drag reparent/reorder, persistence, assistive technology, or packaged behavior. | Mouse/keyboard single/range/toggle select, expand/collapse, rename, search, drag reparent/reorder, scene roots, active state, prefab indicators, focus preservation and undo all pass. |
